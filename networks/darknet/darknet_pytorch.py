...
import torch
from networks.darknet.utils import parse_cfg, print_cfg, load_conv_bn, load_fc, load_conv, get_region_boxes_mix_output
from networks.darknet.region_loss import RegionLoss
from networks.darknet.yolo_layer import YoloLayer
import numpy as np
...
class Darknet_custom(nn.Module):
    def __init__(self, cfgfile, num_bb_filter=None, inference=False):
        super(Darknet_custom, self).__init__()
        self.inference = inference
        self.training = not self.inference
        self.num_output = num_bb_filter
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # return get_region_boxes_custom(out_boxes, num_output=self.num_output)
            return get_region_boxes_mix_output(out_boxes)
