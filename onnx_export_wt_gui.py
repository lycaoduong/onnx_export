...
def convert_yolo7_onnx(cfg_file, weight, input_size=512, save_dir='./export_model', save_name='v7_test', dynamic=False):
    in_s, out_s = yolov7_pt_2_onnx(cfg_file, weight, input_size, save_dir, onnx_file_name=save_name, dynamic=dynamic)
    print('Done. In: {}, Out: {}'.format(in_s, out_s))
    print('Check model on {}'.format(save_dir))
...
if __name__ == '__main__':
    ...
    if version == 4:
        ...
        cfg_file = opt.get('cfg_filev7')
        convert_yolo7_onnx(cfg_file, weight_file_v7, input_size, dynamic=dynamic)
