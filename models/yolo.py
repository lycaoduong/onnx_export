...
import torch
from networks.yolov7_pt.common import *
from networks.yolov7_pt.experimental import *
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        ...
        # return self.ly_modify(x)
        return torch.squeeze(x[0], dim=0)
        # return x

    def ly_modify(self, x):
        x = x[0]
        # x = torch.squeeze(x, dim=0)
        x[:, :, 5:] *= x[:, :, 4:5]
        boxes = x[:, :, :4]
        boxes = torch.squeeze(boxes)
        scores, idx = torch.max(x[:, :, 5:], dim=2)
        det = torch.cat((boxes, scores.permute((1, 0))), dim=1)
        x = torch.cat((det, idx.permute((1, 0))), dim=1)
        return x
